...
public:
	AstSelectList(AstNodeType ast_node_type, bool is_all, AstNode* args, AstNode* next);
	~AstSelectList();
...
public:
	AstSelectExpr(AstNodeType ast_node_type, std::string expr_alias, AstNode* expr);
	~AstSelectExpr();
	void Print(int level = 0) const;
...
public:
	AstTable(AstNodeType ast_node_type, std::string db_name, std::string table_name, std::string table_alias);
	~AstTable();
	void Print(int level = 0) const;
...
public:
	AstSubquery(AstNodeType ast_node_type, std::string subquery_alias, AstNode* subquery);
	~AstSubquery();
	void Print(int level = 0) const;
...
public:
	enum JoinConditionType {
		JOIN_NONE, JOIN_ON, JOIN_USING
	};
	AstJoinCondition(AstNodeType ast_node_type, JoinConditionType join_condition_type, AstNode* condition);
	~AstJoinCondition();
...
public:
	...
	AstJoin(AstNodeType ast_node_type, AstNode* left_node, AstNode* right_node, AstJoinCondition* join_condition);
	~AstJoin();
	void Print(int level = 0) const;
	AstNode* left_node_;
	AstNode* right_node_;
...
public:
	AstGroupByClause(AstNodeType ast_node_type, AstNode* groupby_list, bool with_roolup);
	~AstGroupByClause();
	void Print(int level = 0) const;
...
public:
	...
	explicit AstOrderByList(AstNodeType ast_node_type, AstNode* expr, int orderby_type, AstNode* next);
	~AstOrderByList();
	void Print(int level = 0) const;
	AstNode* expr_;
...
public:
	AstLimitClause(AstNodeType ast_node_type, AstNode* offset, AstNode* row_count);
	~AstLimitClause();
	void Print(int level = 0) const;
	AstNode* offset_;
	AstNode* row_count_;
...
public:
	AstColumn(AstNodeType ast_node_type, std::string relation_name, std::string column_name);
	~AstColumn();
	void Print(int level = 0) const;
...
public:
	enum SelectOpts {
		SELECT_ALL, SELECT_DISTINCT, SELECT_DISTINCTROW,
	};
	AstSelectStmt(AstNodeType ast_node_type, int select_opts, AstNode* select_list, AstNode* from_list, AstNode* where_clause,
			AstNode* groupby_clause, AstNode* having_clause, AstNode* orderby_clause, AstNode* limit_clause,
