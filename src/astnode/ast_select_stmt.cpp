...
#include <iomanip>
using namespace std;
AstSelectList::AstSelectList(AstNodeType ast_node_type, bool is_all, AstNode* args, AstNode* next) :
...
AstSelectExpr::AstSelectExpr(AstNodeType ast_node_type, std::string expr_alias, AstNode* expr) :
...
AstFromList::AstFromList(AstNodeType ast_node_type, AstNode* args, AstNode* next) :
		AstNode(ast_node_type), args_(args), next_(next) {
}
...
AstTable::AstTable(AstNodeType ast_node_type, string db_name, string table_name, string table_alias) :
		AstNode(ast_node_type), db_name_(db_name), table_name_(table_name), table_alias_(table_alias) {
}
...
AstSubquery::AstSubquery(AstNodeType ast_node_type, std::string subquery_alias, AstNode* subquery) :
		AstNode(ast_node_type), subquery_alias_(subquery_alias), subquery_(subquery) {
}
...
void AstSubquery::Print(int level) const {
	cout << setw(level * 8) << " " << "|subquery| " << " subquery_alias: " << subquery_alias_ << endl;
	if(subquery_!=NULL) {
		subquery_->Print(level);
	}
}

AstJoinCondition::AstJoinCondition(AstNodeType ast_node_type, JoinConditionType join_condition_type, AstNode* condition) :
		AstNode(ast_node_type), join_condition_type_(join_condition_type), condition_(condition) {
}
...
AstJoin::AstJoin(AstNodeType ast_node_type, AstNode* left_node, AstNode* right_node, AstJoinCondition* join_condition) :
		AstNode(ast_node_type), left_node_(left_node), right_node_(right_node), join_condition_(join_condition) {
}
...
AstGroupByList::AstGroupByList(AstNodeType ast_node_type, AstNode* expr, AstNode* next) :
...
AstGroupByClause::AstGroupByClause(AstNodeType ast_node_type, AstNode* groupby_list, bool with_roolup) :
		AstNode(ast_node_type), groupby_list_((AstGroupByList*) groupby_list), with_roolup_(with_roolup) {
}
...
void AstGroupByClause::Print(int level) const {
	cout << setw(level * 8) << " " << "|groupby clause| " << "with rool_up: " << with_roolup_ << endl;
	groupby_list_->Print(level+1);
}

AstOrderByList::AstOrderByList(AstNodeType ast_node_type, AstNode* expr, int orderby_type, AstNode* next) :
		AstNode(ast_node_type), expr_(expr), orderby_type_((OrderByType) orderby_type), next_(next) {
}
...
AstOrderByClause::AstOrderByClause(AstNodeType ast_node_type, AstNode* orderby_list) :
		AstNode(ast_node_type), orderby_list_((AstOrderByList*) orderby_list) {
}
...
AstLimitClause::AstLimitClause(AstNodeType ast_node_type, AstNode* offset, AstNode* row_count) :
...
AstColumn::AstColumn(AstNodeType ast_node_type, std::string relation_name, std::string column_name) :
		AstNode(ast_node_type), relation_name_(relation_name), column_name_(column_name) {
}
...
void AstColumn::Print(int level) const {
	...
	level++;
	cout << setw(level * 8) << " " << "relation name: " << relation_name_ << endl;
	cout << setw(level * 8) << " " << "column name: " << column_name_ << endl;
}

AstSelectStmt::AstSelectStmt(AstNodeType ast_node_type, int select_opts, AstNode* select_list, AstNode* from_list, AstNode* where_clause, AstNode* groupby_clause,
		AstNode* having_clause, AstNode* orderby_clause, AstNode* limit_clause, AstNode* select_into_clause) :
		AstNode(ast_node_type), select_opts_((SelectOpts) select_opts), select_list_(select_list), from_list_(from_list), where_clause_(where_clause),
		groupby_clause_(groupby_clause), having_clause_(having_clause), orderby_clause_(orderby_clause), limit_clause_(limit_clause), select_into_clause_(select_into_clause) {
}
